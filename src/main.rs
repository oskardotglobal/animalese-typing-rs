...
impl AnimaleseTyping {
    ...
    pub fn read_key(&mut self, key: Keycode) {
        ...
        if let Some(input) = self.layout.parse_key_chord(&key, &self.modifiers) {
            ...
            {
                Self::VOLUME * Self::SCREAMING_VOLUME_FACTOR
            } else {
                Self::VOLUME
            };
...
fn main() {
    AnimaleseTyping::new(AnimaleseVoice::Female1, Box::new(IsoDeLayout)).run();
}
